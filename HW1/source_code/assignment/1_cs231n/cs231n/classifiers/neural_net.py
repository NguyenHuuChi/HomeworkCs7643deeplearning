...
def two_layer_net(X, model, y=None, reg=0.0):
  ...
  deno = np.reshape(numer.sum(1),(N,1))
  softmax =np.divide(numer,deno)
  softmax+=1e-6

  regu=0.5*reg*(np.sum(np.square(W1))+np.sum(np.square(W2)))
  ...
  grads["W2"] = (1/N) * np.matmul(H1_relu.T,softmax)+reg*W2
