...
class LinearClassifier:
  ...
  def predict(self, X):
    ...
    ###########################################################################
    z = np.dot(self.W,X)  #num_class* N
    z=softmax(z)
    y_pred = np.argmax(y_pred,axis=0) # add argmax instead of max to return the indice --> ( the class )
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_pred
  ...
  def loss(self, X_batch, y_batch, reg):
    ...
    """
    criateria =Softmax()
    loss,grad= criateria.loss(self.W,X_batch, y_batch, reg)
